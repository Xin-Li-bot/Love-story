...
- 心情日历：GitHub 式热力图展示近一年每日心情（5 档色阶，含小记）；前台区块内置打卡表单，**无需登录即可公开添加/更新**（同日 upsert 覆盖），删除仍需管理员。
- 时间胶囊：写给未来的信，公开接口在开启日期前只返回标题与倒计时、隐藏正文（服务端锁），到期后展示正文；前台区块内置封存表单，**无需登录即可公开添加**（含 500 条上限保护），编辑/删除仍需管理员。
...
- 现网实际前端入口为 `static/refactor-app.js` 与 `static/refactor.css`，以 `?v=` 版本号强制刷新（当前 `?v=20260722`，随每次前端发布递增）。
